...
namespace Gst {
	...
	public partial class Iterator : IEnumerable {
		...
		delegate IntPtr d_g_value_reset (ref GLib.Value val);
		static d_g_value_reset g_value_reset = FuncLoader.LoadFunction<d_g_value_reset>(FuncLoader.GetProcAddress(GLibrary.Load(Library.GObject), "g_value_reset"));

		[UnmanagedFunctionPointer (CallingConvention.Cdecl)]
		delegate int d_gst_iterator_next(IntPtr raw, ref GLib.Value elem);
		static d_gst_iterator_next gst_iterator_next = FuncLoader.LoadFunction<d_gst_iterator_next>(FuncLoader.GetProcAddress(GLibrary.Load(Library.Gst), "gst_iterator_next"));
