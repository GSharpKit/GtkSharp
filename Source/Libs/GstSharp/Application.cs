...
namespace Gst {
	...
	partial class Application 
	{
		...
		delegate void d_gst_init_ref(ref int argc, ref IntPtr[] argv);
		static d_gst_init_ref gst_init_ref = FuncLoader.LoadFunction<d_gst_init_ref>(FuncLoader.GetProcAddress(GLibrary.Load(Library.Gst), "gst_init"));

		[UnmanagedFunctionPointer (CallingConvention.Cdecl)]
		delegate void d_gst_init(IntPtr argc, IntPtr argv);
		static d_gst_init gst_init = FuncLoader.LoadFunction<d_gst_init>(FuncLoader.GetProcAddress(GLibrary.Load(Library.Gst), "gst_init"));
		...
		public static void Init(ref string[] argv) {
			...
			gst_init_ref(ref cnt_argv, ref native_argv);
		...
		[UnmanagedFunctionPointer (CallingConvention.Cdecl)]
		delegate bool d_gst_init_check_ref(ref int argc, ref IntPtr[] argv, out IntPtr error);
		static d_gst_init_check_ref gst_init_check_ref = FuncLoader.LoadFunction<d_gst_init_check_ref>(FuncLoader.GetProcAddress(GLibrary.Load(Library.Gst), "gst_init_check"));

		[UnmanagedFunctionPointer (CallingConvention.Cdecl)]
		delegate bool d_gst_init_check(IntPtr argc, IntPtr argv, out IntPtr error);
		static d_gst_init_check gst_init_check = FuncLoader.LoadFunction<d_gst_init_check>(FuncLoader.GetProcAddress(GLibrary.Load(Library.Gst), "gst_init_check"));
		...
		public static bool InitCheck(ref string[] argv) {
			...
			IntPtr[] native_argv = native_arg_list.ToArray();
			IntPtr error = IntPtr.Zero;
			bool ret = gst_init_check_ref(ref cnt_argv, ref native_argv, out error);
			foreach (var native_arg in native_arg_list)
				GLib.Marshaller.Free (native_arg);
			if (error != IntPtr.Zero) throw new GLib.GException (error);
